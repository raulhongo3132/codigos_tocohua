...
int main(){
	...
	string g;	
	int n, i;
	float sumax=0, sumay=0, sumaxy=0, sumaxx=0, ssx=0, ssy=0, spm=0, mx, my, sx, sy, r, b0, b1;
	cout<<endl<<endl<<"\t\tPrograma que calcula un modelo de regreción lineal.";
	cout<<endl<<"\t\tbasado en en un conjutno de datos ";
	...
	cout<<endl<<"\t\t==============================================================================="<<endl;
	for(i=0; i<n; i++){
		cout<<endl<<endl<<"\t\t"<<i+1<<"\t\t"<<x[i]<<"\t\t"<<y[i]<<"\t\t"<<xy[i]<<"\t\t"<<xx[i];
	}
	...
	sx=pow(ssx/(n-1),1.0/2.0);
	sy=pow(ssy/(n-1),1.0/2.0);
	r=spm/((n-1)*sx*sy);
